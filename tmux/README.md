...
状态栏固定在底部，并用标准 Unicode 半圆端点绘制为横跨整行的胶囊；不要求
Nerd Font。从左至右依次为：

- `C-a W编号`：显示 Prefix 和当前 window 编号。
- 常驻的固定宽度状态胶囊：状态优先级为 `PREFIX > COPY > SYNC > ZOOM > NORMAL`，
  所以多个状态同时生效时只显示优先级最高的一项，宽度不会跳动。
- 左对齐的 window 标签胶囊：内容为编号、名称以及 `A`（activity）、`B`（bell）、
...
- [ ] 普通终端和 SSH 中 Kanagawa 真彩色、整条状态栏和各内部胶囊正常。
