# Synchronize tmux zsh panes with GNOME's current system proxy.
# Set TMUX_SYSTEM_PROXY_SYNC=0 in ~/.zshrc.local to opt out.

[[ -n "${TMUX:-}" && "${TMUX_SYSTEM_PROXY_SYNC:-1}" != 0 ]] || return 0
command -v gsettings >/dev/null 2>&1 || return 0

autoload -Uz add-zsh-hook

typeset -gi _TMUX_SYSTEM_PROXY_LAST_SYNC=-1
typeset -gi _TMUX_SYSTEM_PROXY_MANAGED=0

_tmux_system_proxy_unquote() {
  REPLY="${1#\'}"
  REPLY="${REPLY%\'}"
}

_tmux_system_proxy_clear() {
  unset http_proxy https_proxy all_proxy no_proxy
  unset HTTP_PROXY HTTPS_PROXY ALL_PROXY NO_PROXY
  _TMUX_SYSTEM_PROXY_MANAGED=0
}

_tmux_system_proxy_sync() {
  emulate -L zsh
  ...
  (( _TMUX_SYSTEM_PROXY_LAST_SYNC == SECONDS )) && return 0
  ...
  _TMUX_SYSTEM_PROXY_LAST_SYNC=$SECONDS
  ...
  if [[ "$mode" != "'manual'" ]]; then
    (( _TMUX_SYSTEM_PROXY_MANAGED )) && _tmux_system_proxy_clear
    return 0
  fi
  ...
  _tmux_system_proxy_unquote "$http_host"
  http_host="$REPLY"
  ...
  _tmux_system_proxy_unquote "$socks_host"
  socks_host="$REPLY"
...
add-zsh-hook precmd _tmux_system_proxy_sync
add-zsh-hook preexec _tmux_system_proxy_sync
_tmux_system_proxy_sync
