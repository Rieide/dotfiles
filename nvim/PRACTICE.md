...
为保证源码行与 blame 行严格对齐，panel 打开期间会临时关闭源码窗口的
自动换行和折叠；关闭 panel 后会恢复原来的窗口设置。
...
项目搜索优先用 Telescope。结果只需要即时筛选和跳转时，直接在 Telescope
中完成；需要保留一组跨文件结果并逐项处理时，将它们发送到 Quickfix：
...
Telescope 内：
<C-q>       将全部结果发送到 Quickfix 并打开面板
<Tab>       选择或取消当前结果
<S-Tab>     选择或取消当前结果并向上移动
<M-q>       只将已选择的结果发送到 Quickfix 并打开面板
```

Quickfix 列表关闭后结果仍然保留，可以继续顺序处理：

```text
<leader>xQ  打开或关闭 Quickfix 面板
[q / ]q     上一个 / 下一个结果
[Q / ]Q     第一个 / 最后一个结果
j / k       在 Quickfix 面板中移动
<CR>        打开面板中的当前结果
q           关闭 Quickfix 面板
```
...
Quickfix 在这里是持久的结果队列，不替代 Telescope 的交互式筛选、Trouble
的 diagnostics/LSP 展示或 GrugFar 的批量替换。
...
ctags
vimgrep/grep
quickfix history / :cfdo 等进阶 workflow
native completion
