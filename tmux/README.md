...
这套配置面向 Ubuntu Linux 和 tmux 3.3 以上版本，当前由 GNU Stow 链接到
...
| `INSTALLED` | 本次已安装/刷新、生成 completion 或切换到固定 commit |
| `PLANNED` | dry-run 中会执行的动作 |
| `FAILED` | 该项未达到后置条件；其他项目仍会继续 |

退出码 `0` 表示最终表格没有 `FAILED`；`2` 表示所有独立步骤与最终汇总均已完成，
但仍有至少一项 `FAILED`；`1` 保留给参数错误、日志初始化失败或未预期的致命错误。
...
`compinit` 前把该目录加入 `fpath`，并无条件登记 `~/.local/bin` 与 `/snap/bin`，
不要求这些目录在 shell 启动时已经存在。

### 插件目录安全策略

所有 tmux 插件固定安装在 `~/.config/tmux/plugins`（即使设置了自定义
`XDG_CONFIG_HOME`），插件源码不纳入 dotfiles Git：

- 目录缺失时，先 clone 到同一文件系统的临时目录，checkout 固定 commit，成功后
  再原子移动到目标位置。
- origin 等价于对应官方 GitHub 仓库、工作区干净且 HEAD 正确时跳过；接受标准
  HTTPS、GitHub SSH，以及旧版 TPM 生成的
  `https://git::@github.com/<owner>/<repo>`，其他带凭据 URL 仍拒绝。
...
TPM 仅负责按声明顺序加载插件。启动 TPM 时只为该子进程设置
`XDG_CONFIG_HOME=$HOME/.config`，因此即使会话使用自定义 XDG 目录，它仍会读取
`~/.config/tmux/tmux.conf` 中的声明和固定插件目录，不会改写 tmux server 或用户
会话的全局 XDG 环境。其安装、更新和清理键 `Prefix+I`、`Prefix+U`、
...
状态栏固定在底部，使用纯文本和矩形色块，不使用 Nerd Font 或 Unicode 半圆字符。
从左至右依次为：

- 当前 session 名称（即 `tmux attach`/`tmux detach` 操作的会话）。
- 常驻的固定宽度状态字段：状态优先级为 `PREFIX > COPY > SYNC > ZOOM > NORMAL`，
  所以多个状态同时生效时只显示优先级最高的一项，宽度不会跳动。
- 左对齐的 window 标签：内容为编号、名称以及 `A`（activity）、`B`（bell）、
  `Z`（zoom）标志；当前标签使用 Kanagawa 蓝色强调。
- 最多 40 字符的当前 pane 完整路径，然后是 `YYYY-MM-DD HH:MM`。
...
picker 每次启动都会把 `~/.local/bin` 与 `/snap/bin` 补入自己的 `PATH`，所以即使
tmux server 早于 sesh 启动，安装完成后也不需要为 PATH 重启 server。
...
shell 与 copy mode 的无前缀导航在 zoom 状态下也调用同一 helper，并显式传入当前
pane id；普通状态继续使用 tmux 原生命令，避免为常规导航启动额外进程。
...
共享 shell hook 在 zsh/bash 每次显示 prompt 时把该 pane 的 conda 状态写入 tmux
pane 选项；zsh 还会通过导出的 `PROMPT_COMMAND` 让后续启动的 bash 自动继承追踪器，
而无需接管 `~/.bashrc`。resurrect 保存布局后把这些 pane 选项追加到状态文件；恢复完
pane、scrollback 和 bash 后，再逐 pane 执行对应的 `conda activate`。`base`、命名
环境和未激活状态分别保存；某个环境缺失时只在对应 pane 显示提示，不影响其他 pane。

快照和 pane 内容固定保存在 `~/.local/share/tmux/resurrect`，位于 dotfiles 仓库之外，
不会随本仓库提交或上传 GitHub。pane 内容没有加密，且每个 pane 最多可能保存
`history-limit` 指定的 50,000 行；其中可能包含终端输出里的路径或敏感文本。每个 pane
的 conda 环境名称也只存在该目录的 resurrect 状态文件中。
...
tmux 配置仍能加载，但手动保存/恢复键和自动保存/恢复不可用。安装插件后执行
`Prefix+r` 即可让当前 server 重新加载 TPM 并检查绑定；只有验证 continuum 的
“新 server 启动时自动恢复”行为时才需要启动新 server。sesh 安装后的 PATH 生效
不属于插件重载场景，无需重启。
...
tmux show-options -g @resurrect-hook-post-save-layout
tmux show-options -g @resurrect-hook-post-restore-all
...
dry-run 返回 `2` 表示命令已完成并打印汇总，但需要审查其中的 `FAILED` 行；验收通过
仍要求返回 `0`。返回 `1` 表示未正常完成汇总的致命错误。
